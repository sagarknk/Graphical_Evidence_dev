...
parfor lambda_idx = 1:length(lambda_vec)
    ...
    lambda_inv = lambda_vec(1,lambda_idx);
    %lambda = 1/lambda_inv;
    lambda = lambda_inv;
    ...
    csvwrite(['./True_Omega_mat/Omega_q_',num2str(q),'_lambda_',num2str(lambda_inv),'.csv'], True_Omega);

    for n = sample_size
        xx= mvnrnd(zeros(1,q),inv(True_Omega),n);
        S = xx'*xx;
        csvwrite(['./X_mat/X_mat_q_',num2str(q),'_n_',num2str(n),'_lambda_',num2str(lambda_inv),'.csv'], xx);
        csvwrite(['./S_mat/S_mat_q_',num2str(q),'_n_',num2str(n),'_lambda_',num2str(lambda_inv),'.csv'], S);
    end
