function OSP_Real_data_BGL_graphical_evidence(lambda)
...
xx = xx_all(1:20,:);
xx_test = xx_all(21:n_all, :);

[n,q] = size(xx);
%S = xx'*xx;

Matrix_of_log_ratio_likelihoods = zeros(1, q);
...
try
    random_order = 1:q;
    fprintf("The value of lambda is : %f\n", lambda);
    fprintf("Marginal likelihood is being computed for %f lambda\n", lambda);
    ...
    for num_BGL = 1:q
        ...
        if num_BGL<= q-1
            ...
            %
            %    Harmonic_mean_est_vec(1) = Harmonic_mean_est;
            %else
            %    % do nothing
            %end
            ...
            log_normal_posterior_density(1, num_BGL) = ...
                log(mean(exp(vec_log_normal_density)));

            log_gamma_posterior_density(1, num_BGL) = ...
            ...
            log_data_likelihood(1,num_BGL) = LOG_data_density;
            log_posterior_density(1, num_BGL) = LOG_posterior_density;
            ...
            Matrix_of_log_ratio_likelihoods(1,:) = log_ratio_of_liklelihoods;
        end
    end
catch
    fprintf("lambda = %f is skipped\n", lambda);
end
toc;
...
fprintf("%f is the log_marginal at %f lambda\n", All_mle_at_lambda, lambda);

%%%%%%%%%%%%%%%%%%% Out of sample prediction %%%%%%%%%%%%%%%%%
Coeff_matrix = Reconstructed_matrix;

for diag_idx = 1:q
    Coeff_matrix(diag_idx, diag_idx)=0;
    Coeff_matrix(:, diag_idx) = Coeff_matrix(:,diag_idx)/Reconstructed_matrix(diag_idx, diag_idx);
    Coeff_matrix(:, diag_idx) = -1.*Coeff_matrix(:, diag_idx);
end

Est_matrix = xx_test * Coeff_matrix;
Est_norm = norm(Est_matrix - xx_test, "fro");
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf("%f is the prediction norm\n", Est_norm);
fprintf("%f is the prediction norm\n", sqrt(sum(sum((Est_matrix - xx_test).^2))));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
