...
%%% Generating a nxq data matrix from T(0, t_dof,\Sigma = inv(True_Omega))
...
if q==2
    ...
    for outer_gam_idx = 1:1e4
        ...
        %if mod(outer_gam_idx,1e2)==0
        %    fprintf("%d iters done",outer_gam_idx);
        %end
        ...
        outer_vals_for_expectation(1,outer_gam_idx) = ...
            prior_u_(1,outer_gam_idx)^n .* exp(constant_part_log_marginal + integral_part_log_marginal);
    end

    log_marginal = -log(0.67) + log(mean(outer_vals_for_expectation));
    ...
    for num_BGL = 1:q %%% consider num_BGL equivalent to index j in the paper
        ...
        if num_BGL<= q-1
            ...
            [post_mean_omega,post_mean_tau_save, MC_average_Equation_9] = ...
                BGL_Hao_wang(S_reduced,n,burnin,nmc,lambda, t_dof, Matrix_2be_added_Gibbs);
            ...
            [post_mean_omega_22_2ndGibbs, MC_average_Equation_11] = ...
                BGL_last_col_fixed(S_reduced,n,burnin, nmc,lambda, t_dof,fixed_last_col, ...
            ...
            if 0.5*(n + t_dof) < 172
                ...
                log_data_likelihood(1, num_BGL) = log(gamma(0.5*(n+t_dof))) - log(gamma(0.5*t_dof)) -0.5*n*log(t_dof) ...
                ...
                log_data_likelihood(1, num_BGL) = log(mean(temp_norm_pdf));
            ...
            temp_omega_sample =  gamrnd(n/2 + 1, 2/(lambda +  S_reduced));
            for i = 1:nmc
                ...
                temp_omega_sample = gamrnd(n/2 + 1, 2/(lambda + posterior_u_ .* S_reduced));
            ...
            for i=1:nmc
                temp_log_posterior_density(1,i) = gampdf(post_mean_omega_direct, ...
                    n/2+1, 2/(lambda+ posterior_u_save(1,i).*S_reduced));
            end
