...
parfor skilling_iter = 1:25
    ...
    %%% prior_sampling samples from the BGL prior 
    [~,prior_samples, tau_samples] = prior_sampling(q, burnin,nmc, lambda, skilling_iter);
    ...
    for i = 1:J
        ...
        tau_for_omega_sample = tau_samples(:,:,rank_low_ll);
        
        while new_greater_likelihood < lowest_likelihood

            [temp_omega_sample,tau_for_omega_sample] = prior_sampling_for_Neal_and_skilling(q, lambda, ...
                temp_omega_sample, tau_for_omega_sample);
